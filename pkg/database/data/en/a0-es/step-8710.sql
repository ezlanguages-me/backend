...
-- ============================================================
    ...
    DECLARE
        ...
        v_exercises JSONB[] := ARRAY[
            '{"p": "The dialogue is about a refund request for a faulty blender bought two days ago.", "p_de": "The dialogue is about a refund request for a faulty blender bought two days ago.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            ...
            '{"p": "Clerk explains that the customer has a receipt and the blender stops after one minute.", "p_de": "Clerk explains that the customer has a receipt and the blender stops after one minute.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            '{"p": "A counterpoint is that the box is open but all parts are present.", "p_de": "A counterpoint is that the box is open but all parts are present.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            ...
            '{"p": "What is the main focus?", "p_de": "What is the main focus?", "s": {"type": "multiple_choice", "options": ["a refund request for a faulty blender bought two days ago", "a broken bus stop", "a lost suitcase"], "answer": 0}}'::jsonb,
            ...
            '{"p": "Which detail is named?", "p_de": "Which detail is named?", "s": {"type": "multiple_choice", "options": ["the customer has a receipt and the blender stops after one minute", "the door is painted blue", "the train leaves at midnight"], "answer": 0}}'::jsonb,
            '{"p": "Which counterpoint appears?", "p_de": "Which counterpoint appears?", "s": {"type": "multiple_choice", "options": ["the box is open but all parts are present", "nobody has a notebook", "the lesson is cancelled"], "answer": 0}}'::jsonb,
            '{"p": "What conclusion do the speakers reach?", "p_de": "What conclusion do the speakers reach?", "s": {"type": "multiple_choice", "options": ["the store offers a full refund or an exchange", "they forget the topic completely", "they decide to leave the room"], "answer": 0}}'::jsonb
        ];
    BEGIN
...
DELETE FROM writing WHERE step_order=8710 AND path_uuid=v_path_id;
        ...
        VALUES (v_path_id, 8710, 'en', 'dialogue', 'shopping', '[{"name": "Customer", "gender": "neutral", "avatarURL": "https://example.com/avatars/customer.png"}, {"name": "Clerk", "gender": "neutral", "avatarURL": "https://example.com/avatars/clerk.png"}]'::jsonb)RETURNING uuid INTO v_dialogue_id;
        INSERT INTO dialogue_translation (dialogue_uuid, language, title) VALUES (v_dialogue_id, 'es', 'ask for a refund or exchange');
        INSERT INTO dialogue_translation (dialogue_uuid, language, title) VALUES (v_dialogue_id, 'de', 'ask for a refund or exchange');
