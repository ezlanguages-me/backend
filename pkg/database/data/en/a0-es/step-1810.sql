...
BEGIN
    ...
    VALUES (v_path_id, 1810, 'en', 'speaking', 'shopping')RETURNING uuid INTO v_speaking_id;

    INSERT INTO speaking_translation (speaking_uuid, language, title, prompt)
    VALUES (v_speaking_id, 'es', 'Comunicación con gestos', '{"scenario": "Estás en un mercado al aire libre. Quieres comprar uno o dos productos sencillos y pedir un precio un poco mejor usando palabras básicas y gestos.", "tasks": ["Di qué producto quieres comprar.", "Pregunta el precio con una frase corta.", "Usa los dedos para mostrar la cantidad.", "Pide un precio un poco más bajo.", "Pregunta si el vendedor puede incluir una bolsa.", "Di cuánto quieres pagar.", "Acepta o rechaza con una frase muy simple.", "Termina con gracias."]}'::jsonb);
    INSERT INTO speaking_translation (speaking_uuid, language, title, prompt)
    VALUES (v_speaking_id, 'de', 'Kommunikation mit Gesten', '{"scenario": "Du bist auf einem offenen Markt. Du möchtest ein oder zwei einfache Produkte kaufen und mit einfachen Worten und Gesten um einen etwas besseren Preis bitten.", "tasks": ["Sage, welches Produkt du kaufen möchtest.", "Frage mit einem kurzen Satz nach dem Preis.", "Zeige die Menge mit den Fingern.", "Bitte um einen etwas niedrigeren Preis.", "Frage, ob der Verkäufer eine Tasche dazugibt.", "Sage, wie viel du zahlen möchtest.", "Nimm an oder lehne mit einem sehr einfachen Satz ab.", "Beende mit einem Dank."]}'::jsonb);
END;
$seed$;
