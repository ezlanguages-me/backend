-- ============================================================
-- Seed: A0 English Path – STEP 6650 – Listening – listen to tourist information clarification (Turismo y Entretenimiento)
...
BEGIN
    ...
    DELETE FROM exercise WHERE target_uuid IN (SELECT uuid FROM reading WHERE step_order=6650 AND path_uuid=v_path_id);
    DELETE FROM exercise WHERE target_uuid IN (SELECT uuid FROM listening WHERE step_order=6650 AND path_uuid=v_path_id);
    DELETE FROM exercise WHERE target_uuid IN (SELECT uuid FROM dialogue WHERE step_order=6650 AND path_uuid=v_path_id);
    DELETE FROM reading WHERE step_order=6650 AND path_uuid=v_path_id;
    DELETE FROM listening WHERE step_order=6650 AND path_uuid=v_path_id;
    DELETE FROM dialogue WHERE step_order=6650 AND path_uuid=v_path_id;
    DELETE FROM speaking WHERE step_order=6650 AND path_uuid=v_path_id;
    DELETE FROM writing WHERE step_order=6650 AND path_uuid=v_path_id;
    INSERT INTO listening (path_uuid,step_order,source_language,type,category,transcript)
    VALUES (v_path_id,6650,'en','listening','tourism',$transcript$# AUDIO PROFILE
One guide speaks clearly to a small group of visitors.

## THE SCENE
A short visit connected to Old Quarter Visitor Info.
...
### SAMPLE CONTEXT
A visitor listens to practical information about Old Quarter Visitor Info.

#### TRANSCRIPT
[warm] Guide: Welcome to Old Quarter Visitor Info in the central station area.
[clear] Guide: We start at the information desk at 08:30.
[calm] Guide: First, you can see the walking map.
[informative] Guide: Later, we continue to the bus pass desk.
[steady] Guide: The visit lasts 15 minutes.
[helpful] Guide: The price is free.
[friendly] Guide: Please check the colour of your map.
[calm] Guide: At the end, the tram stop is nearby.
$transcript$)
    RETURNING uuid INTO v_listening_id;
    INSERT INTO listening_translation (listening_uuid,language,title,description) VALUES (v_listening_id,'es','Aclaraciones de información turística','Escucha la aclaración y responde.');
    INSERT INTO listening_translation (listening_uuid,language,title,description) VALUES (v_listening_id,'de','Klärungen der Touristeninformation','Höre die Klärung und antworte.');
