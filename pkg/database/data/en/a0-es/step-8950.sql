...
-- ============================================================
    ...
    DECLARE
        ...
        v_exercises JSONB[] := ARRAY[
            '{"p": "The dialogue is about the manager contacts the legal team to clarify the liability clause in a contract.", "p_de": "The dialogue is about the manager contacts the legal team to clarify the liability clause in a contract.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            ...
            '{"p": "Legal Adviser explains that the clause limits the company''s liability to the value of the original contract.", "p_de": "Legal Adviser explains that the clause limits the company''s liability to the value of the original contract.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            '{"p": "A counterpoint is that the adviser notes that local law may impose additional obligations.", "p_de": "A counterpoint is that the adviser notes that local law may impose additional obligations.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            ...
            '{"p": "What is the main focus?", "p_de": "What is the main focus?", "s": {"type": "multiple_choice", "options": ["the manager contacts the legal team to clarify the liability clause in a contract", "a broken bus stop", "a lost suitcase"], "answer": 0}}'::jsonb,
            '{"p": "Who gives the main explanation?", "p_de": "Who gives the main explanation?", "s": {"type": "multiple_choice", "options": ["Legal Adviser", "the cleaner", "the taxi driver"], "answer": 0}}'::jsonb,
            '{"p": "Which detail is named?", "p_de": "Which detail is named?", "s": {"type": "multiple_choice", "options": ["the clause limits the company''s liability to the value of the original contract", "the door is painted blue", "the train leaves at midnight"], "answer": 0}}'::jsonb,
            '{"p": "Which counterpoint appears?", "p_de": "Which counterpoint appears?", "s": {"type": "multiple_choice", "options": ["the adviser notes that local law may impose additional obligations", "nobody has a notebook", "the lesson is cancelled"], "answer": 0}}'::jsonb,
            '{"p": "What conclusion do the speakers reach?", "p_de": "What conclusion do the speakers reach?", "s": {"type": "multiple_choice", "options": ["the manager requests a written summary before signing the agreement", "they forget the topic completely", "they decide to leave the room"], "answer": 0}}'::jsonb
        ];
    BEGIN
...
DELETE FROM writing WHERE step_order=8950 AND path_uuid=v_path_id;
        ...
        VALUES (v_path_id, 8950, 'en', 'dialogue', 'professional', '[{"name": "Customer", "gender": "neutral", "avatarURL": "https://example.com/avatars/customer.png"}, {"name": "Legal Adviser", "gender": "neutral", "avatarURL": "https://example.com/avatars/clerk.png"}]'::jsonb)RETURNING uuid INTO v_dialogue_id;
        INSERT INTO dialogue_translation (dialogue_uuid, language, title) VALUES (v_dialogue_id, 'es', 'Outside own immediate area of');
        INSERT INTO dialogue_translation (dialogue_uuid, language, title) VALUES (v_dialogue_id, 'de', 'Outside own immediate area of');
