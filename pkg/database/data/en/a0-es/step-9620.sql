    ...
    DECLARE
        ...
        v_exercises JSONB[] := ARRAY[
            '{"p": "The dialogue is about two colleagues discuss an ironic situation at work with British humour.", "p_de": "The dialogue is about two colleagues discuss an ironic situation at work with British humour.", "s": {"type": "true_false", "answer": true}}'::jsonb,
        ...
        '{"p": "Colleague explains that the manager who always complained about meetings has now scheduled three extra ones.", "p_de": "Colleague explains that the manager who always complained about meetings has now scheduled three extra ones.", "s": {"type": "true_false", "answer": true}}'::jsonb,
        '{"p": "A counterpoint is that one colleague jokes that the manager probably ran out of emails to ignore.", "p_de": "A counterpoint is that one colleague jokes that the manager probably ran out of emails to ignore.", "s": {"type": "true_false", "answer": true}}'::jsonb,
        ...
        '{"p": "What is the main focus?", "p_de": "What is the main focus?", "s": {"type": "multiple_choice", "options": ["two colleagues discuss an ironic situation at work with British humour", "a broken bus stop", "a lost suitcase"], "answer": 0}}'::jsonb,
        ...
        '{"p": "Which detail is named?", "p_de": "Which detail is named?", "s": {"type": "multiple_choice", "options": ["the manager who always complained about meetings has now scheduled three extra ones", "the door is painted blue", "the train leaves at midnight"], "answer": 0}}'::jsonb,
        '{"p": "Which counterpoint appears?", "p_de": "Which counterpoint appears?", "s": {"type": "multiple_choice", "options": ["one colleague jokes that the manager probably ran out of emails to ignore", "nobody has a notebook", "the lesson is cancelled"], "answer": 0}}'::jsonb,
        '{"p": "What conclusion do the speakers reach?", "p_de": "What conclusion do the speakers reach?", "s": {"type": "multiple_choice", "options": ["they laugh and agree the situation is a perfect example of British workplace irony", "they forget the topic completely", "they decide to leave the room"], "answer": 0}}'::jsonb
        ];
    BEGIN
...
DELETE FROM writing WHERE step_order=9620 AND path_uuid=v_path_id;
        ...
        VALUES (v_path_id, 9620, 'en', 'dialogue', 'social', '[{"name": "Speaker A", "gender": "neutral", "avatarURL": "https://example.com/avatars/speaker-a.png"}, {"name": "Colleague", "gender": "neutral", "avatarURL": "https://example.com/avatars/colleague.png"}]'::jsonb)RETURNING uuid INTO v_dialogue_id;

        INSERT INTO dialogue_translation (dialogue_uuid, language, title) VALUES (v_dialogue_id, 'es', 'Diálogo con adecuación');
        INSERT INTO dialogue_translation (dialogue_uuid, language, title) VALUES (v_dialogue_id, 'de', 'Angemessener Dialog');
