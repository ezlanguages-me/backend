...
-- ============================================================
    ...
    DECLARE
        ...
        v_exercises JSONB[] := ARRAY[
            '{"p": "The dialogue is about a speaker responding calmly to hostile questioning after a project presentation.", "p_de": "The dialogue is about a speaker responding calmly to hostile questioning after a project presentation.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            '{"p": "The speakers are discussing the colour of the presentation slides.", "p_de": "The speakers are discussing the colour of the presentation slides.", "s": {"type": "true_false", "answer": false}}'::jsonb,
            '{"p": "The questioner demands a direct explanation for the increased budget.", "p_de": "The questioner demands a direct explanation for the increased budget.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            '{"p": "The speaker explains that supply chain problems raised costs during the project.", "p_de": "The speaker explains that supply chain problems raised costs during the project.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            '{"p": "The exchange ends when the audience accepts the honest explanation.", "p_de": "The exchange ends when the audience accepts the honest explanation.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            '{"p": "What is the main focus?", "p_de": "What is the main focus?", "s": {"type": "multiple_choice", "options": ["a speaker answers a hostile question about the project budget", "an unrelated social chat", "a routine travel update"], "answer": 0}}'::jsonb,
            '{"p": "Who gives the budget explanation?", "p_de": "Who gives the budget explanation?", "s": {"type": "multiple_choice", "options": ["Speaker", "the cleaner", "the driver"], "answer": 0}}'::jsonb,
            '{"p": "Which detail is named?", "p_de": "Which detail is named?", "s": {"type": "multiple_choice", "options": ["the questioner demands a direct budget explanation", "the room is too cold", "the event starts late"], "answer": 0}}'::jsonb,
            '{"p": "What answer is given?", "p_de": "What answer is given?", "s": {"type": "multiple_choice", "options": ["supply chain problems caused the extra cost", "the sound system fails", "the audience falls asleep"], "answer": 0}}'::jsonb,
            '{"p": "What conclusion do the speakers reach?", "p_de": "What conclusion do the speakers reach?", "s": {"type": "multiple_choice", "options": ["the audience accepts the honest explanation", "the topic is cancelled", "everyone leaves without speaking"], "answer": 0}}'::jsonb
        ];
    BEGIN
...
DELETE FROM writing WHERE step_order=9400 AND path_uuid=v_path_id;
        ...
        VALUES (v_path_id, 9400, 'en', 'dialogue', 'academic', '[{"name": "Speaker", "gender": "neutral", "avatarURL": "https://example.com/avatars/speaker.png"}, {"name": "Questioner", "gender": "neutral", "avatarURL": "https://example.com/avatars/questioner.png"}]'::jsonb)RETURNING uuid INTO v_dialogue_id;
        INSERT INTO dialogue_translation (dialogue_uuid, language, title) VALUES (v_dialogue_id, 'es', 'Deal with hostile questioning');
        INSERT INTO dialogue_translation (dialogue_uuid, language, title) VALUES (v_dialogue_id, 'de', 'Deal with hostile questioning');
