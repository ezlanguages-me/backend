...
-- ============================================================
    ...
    DECLARE
        ...
        v_exercises JSONB[] := ARRAY[
            '{"p": "The dialogue is about the host guides the visitor through the company''s main departments.", "p_de": "The dialogue is about the host guides the visitor through the company''s main departments.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            ...
            '{"p": "Host explains that the visitor is particularly interested in the production and quality control team.", "p_de": "Host explains that the visitor is particularly interested in the production and quality control team.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            '{"p": "A counterpoint is that the visitor raises a concern about the delivery timeline for the next order.", "p_de": "A counterpoint is that the visitor raises a concern about the delivery timeline for the next order.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            ...
            '{"p": "What is the main focus?", "p_de": "What is the main focus?", "s": {"type": "multiple_choice", "options": ["the host guides the visitor through the company''s main departments", "a broken bus stop", "a lost suitcase"], "answer": 0}}'::jsonb,
            ...
            '{"p": "Which detail is named?", "p_de": "Which detail is named?", "s": {"type": "multiple_choice", "options": ["the visitor is particularly interested in the production and quality control team", "the door is painted blue", "the train leaves at midnight"], "answer": 0}}'::jsonb,
            '{"p": "Which counterpoint appears?", "p_de": "Which counterpoint appears?", "s": {"type": "multiple_choice", "options": ["the visitor raises a concern about the delivery timeline for the next order", "nobody has a notebook", "the lesson is cancelled"], "answer": 0}}'::jsonb,
            '{"p": "What conclusion do the speakers reach?", "p_de": "What conclusion do the speakers reach?", "s": {"type": "multiple_choice", "options": ["the host arranges a follow-up call and provides a detailed schedule", "they forget the topic completely", "they decide to leave the room"], "answer": 0}}'::jsonb
        ];
    BEGIN
...
DELETE FROM writing WHERE step_order=9030 AND path_uuid=v_path_id;
        ...
        VALUES (v_path_id, 9030, 'en', 'dialogue', 'professional', '[{"name": "Visitor", "gender": "neutral", "avatarURL": "https://example.com/avatars/customer.png"}, {"name": "Host", "gender": "neutral", "avatarURL": "https://example.com/avatars/clerk.png"}]'::jsonb)RETURNING uuid INTO v_dialogue_id;
        INSERT INTO dialogue_translation (dialogue_uuid, language, title) VALUES (v_dialogue_id, 'es', 'Diálogo con un visitante');
        INSERT INTO dialogue_translation (dialogue_uuid, language, title) VALUES (v_dialogue_id, 'de', 'Dialog mit einem Besucher');
