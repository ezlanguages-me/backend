...
DELETE FROM writing WHERE step_order=9250 AND path_uuid=v_path_id;
        ...
        VALUES (v_path_id, 9250, 'en', 'speaking', 'professional')RETURNING uuid INTO v_speaking_id;
        INSERT INTO speaking_translation (speaking_uuid, language, title, prompt)
        VALUES (v_speaking_id, 'es', 'Standard accent', '{"scenario": "Debes trabajar en inglés sobre este tema: the caller manages a difficult phone call about a delayed delivery. El contexto es a professional phone call and logistics office.", "tasks": ["Presenta el problema o punto de partida.", "Explica tu postura principal.", "Menciona un detalle o evidencia concreta.", "Reconoce el punto de vista contrario.", "Responde al punto contrario de forma respetuosa.", "Refuerza tu argumento con otro ejemplo.", "Resume tu postura de forma clara.", "Cierra con confianza y educación."]}'::jsonb);
        INSERT INTO speaking_translation (speaking_uuid, language, title, prompt)
        VALUES (v_speaking_id, 'de', 'Standard accent', '{"scenario": "Du sollst auf Englisch zu diesem Thema arbeiten: the caller manages a difficult phone call about a delayed delivery. Der Kontext ist a professional phone call and logistics office.", "tasks": ["Stelle das Problem oder den Ausgangspunkt vor.", "Erkläre deine Hauptposition.", "Nenne ein konkretes Detail oder einen Beweis.", "Erkenne den gegenteiligen Standpunkt an.", "Antworte respektvoll auf den Gegenstandpunkt.", "Stärke dein Argument mit einem weiteren Beispiel.", "Fasse deine Position klar zusammen.", "Beende selbstbewusst und höflich."]}'::jsonb);
    END; $seed$;
