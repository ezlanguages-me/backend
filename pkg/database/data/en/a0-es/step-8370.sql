...
-- ============================================================
    ...
    DECLARE
        ...
        v_exercises JSONB[] := ARRAY[
            '{"p": "The dialogue is about whether phones should be banned in seminars.", "p_de": "The dialogue is about whether phones should be banned in seminars.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            ...
            '{"p": "Diego explains that one side argues for attention while the other cites accessibility and quick research.", "p_de": "Diego explains that one side argues for attention while the other cites accessibility and quick research.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            '{"p": "A counterpoint is that the chair asks for evidence instead of slogans.", "p_de": "A counterpoint is that the chair asks for evidence instead of slogans.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            ...
            '{"p": "What is the main focus?", "p_de": "What is the main focus?", "s": {"type": "multiple_choice", "options": ["whether phones should be banned in seminars", "a broken bus stop", "a lost suitcase"], "answer": 0}}'::jsonb,
            ...
            '{"p": "Which detail is named?", "p_de": "Which detail is named?", "s": {"type": "multiple_choice", "options": ["one side argues for attention while the other cites accessibility and quick research", "the door is painted blue", "the train leaves at midnight"], "answer": 0}}'::jsonb,
            '{"p": "Which counterpoint appears?", "p_de": "Which counterpoint appears?", "s": {"type": "multiple_choice", "options": ["the chair asks for evidence instead of slogans", "nobody has a notebook", "the lesson is cancelled"], "answer": 0}}'::jsonb,
            '{"p": "What conclusion do the speakers reach?", "p_de": "What conclusion do the speakers reach?", "s": {"type": "multiple_choice", "options": ["the audience prefers limited use rather than a total ban", "they forget the topic completely", "they decide to leave the room"], "answer": 0}}'::jsonb
        ];
    BEGIN
...
DELETE FROM writing WHERE step_order=8370 AND path_uuid=v_path_id;
        ...
        VALUES (v_path_id, 8370, 'en', 'dialogue', 'academic', '[{"name": "Sara", "gender": "female", "avatarURL": "https://example.com/avatars/sara.png"}, {"name": "Diego", "gender": "male", "avatarURL": "https://example.com/avatars/diego.png"}]'::jsonb)RETURNING uuid INTO v_dialogue_id;
        INSERT INTO dialogue_translation (dialogue_uuid, language, title) VALUES (v_dialogue_id, 'es', 'debate a topic');
        INSERT INTO dialogue_translation (dialogue_uuid, language, title) VALUES (v_dialogue_id, 'de', 'debate a topic');
