-- ============================================================
-- Seed: A0 English Path – STEP 8060 – Dialogue – analyze an argument together (Conferencias, Seminarios y Clases)
...
DELETE FROM exercise WHERE target_uuid IN (SELECT uuid FROM reading WHERE step_order=8060 AND path_uuid=v_path_id);
DELETE FROM exercise WHERE target_uuid IN (SELECT uuid FROM listening WHERE step_order=8060 AND path_uuid=v_path_id);
DELETE FROM exercise WHERE target_uuid IN (SELECT uuid FROM dialogue WHERE step_order=8060 AND path_uuid=v_path_id);
DELETE FROM reading WHERE step_order=8060 AND path_uuid=v_path_id;
DELETE FROM listening WHERE step_order=8060 AND path_uuid=v_path_id;
...
DELETE FROM writing WHERE step_order=8060 AND path_uuid=v_path_id;
        ...
        VALUES
            ...
            (v_dialogue_id, 1, 'Diego', 'Yes. Rosa says that charts from three weeks show longer visits in the quiet zone'),
            (v_dialogue_id, 2, 'Sara', 'So the counterpoint is that students also need bookable tutor desks for group questions?'),
            ...
            (v_dialogue_id, 5, 'Diego', 'They conclude that the presenter recommends keeping the quiet zones and adding a booking system'),
            (v_dialogue_id, 6, 'Sara', 'That makes the explanation clearer for the seminar.'),
            (v_dialogue_id, 7, 'Diego', 'I agree. The best line is: "Analysis is useful when each claim is tied to evidence."');
        INSERT INTO dialogue_lines_translation (dialogue_line_uuid, language, meaning)
        VALUES
            ((SELECT uuid FROM dialogue_lines WHERE dialogue_uuid = v_dialogue_id AND line_order = 0), 'es', '[{"translations": [{"languageCode": "es", "translation": "¿Leíste the note about an analysis of library usage data after new quiet zones were created for a study skills course?"}]}]'::jsonb),
            ((SELECT uuid FROM dialogue_lines WHERE dialogue_uuid = v_dialogue_id AND line_order = 0), 'de', '[{"translations": [{"languageCode": "de", "translation": "Hast du the note about an analysis of library usage data after new quiet zones were created for a study skills course?"}]}]'::jsonb),
            ((SELECT uuid FROM dialogue_lines WHERE dialogue_uuid = v_dialogue_id AND line_order = 1), 'es', '[{"translations": [{"languageCode": "es", "translation": "Sí. Rosa says that charts from three weeks show longer visits in the quiet zone"}]}]'::jsonb),
            ((SELECT uuid FROM dialogue_lines WHERE dialogue_uuid = v_dialogue_id AND line_order = 1), 'de', '[{"translations": [{"languageCode": "de", "translation": "Ja. Rosa says that charts from three weeks show longer visits in the quiet zone"}]}]'::jsonb),
            ((SELECT uuid FROM dialogue_lines WHERE dialogue_uuid = v_dialogue_id AND line_order = 2), 'es', '[{"translations": [{"languageCode": "es", "translation": "So the counterpoint is that students also need bookable tutor desks for group questions?"}]}]'::jsonb),
            ((SELECT uuid FROM dialogue_lines WHERE dialogue_uuid = v_dialogue_id AND line_order = 2), 'de', '[{"translations": [{"languageCode": "de", "translation": "So the counterpoint is that students also need bookable tutor desks for group questions?"}]}]'::jsonb),
            ...
            ((SELECT uuid FROM dialogue_lines WHERE dialogue_uuid = v_dialogue_id AND line_order = 5), 'es', '[{"translations": [{"languageCode": "es", "translation": "They conclude that the presenter recommends keeping the quiet zones and adding a booking system"}]}]'::jsonb),
            ((SELECT uuid FROM dialogue_lines WHERE dialogue_uuid = v_dialogue_id AND line_order = 5), 'de', '[{"translations": [{"languageCode": "de", "translation": "They conclude that the presenter recommends keeping the quiet zones and adding a booking system"}]}]'::jsonb),
            ...
            ((SELECT uuid FROM dialogue_lines WHERE dialogue_uuid = v_dialogue_id AND line_order = 7), 'es', '[{"translations": [{"languageCode": "es", "translation": "Estoy de acuerdo. The best line is: \"Analysis is useful when each claim is tied to evidence.\""}]}]'::jsonb),
            ((SELECT uuid FROM dialogue_lines WHERE dialogue_uuid = v_dialogue_id AND line_order = 7), 'de', '[{"translations": [{"languageCode": "de", "translation": "Ich stimme zu. The best line is: \"Analysis is useful when each claim is tied to evidence.\""}]}]'::jsonb);
