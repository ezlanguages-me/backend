...
-- ============================================================
    ...
    DECLARE
        ...
        v_exercises JSONB[] := ARRAY[
            '{"p": "The dialogue is about the HR officer explains the process for requesting extended leave.", "p_de": "The dialogue is about the HR officer explains the process for requesting extended leave.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            ...
            '{"p": "HR Officer explains that the employee must submit a formal written request at least four weeks in advance.", "p_de": "HR Officer explains that the employee must submit a formal written request at least four weeks in advance.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            '{"p": "A counterpoint is that the officer notes that urgent cases can be handled under a different policy.", "p_de": "A counterpoint is that the officer notes that urgent cases can be handled under a different policy.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            ...
            '{"p": "What is the main focus?", "p_de": "What is the main focus?", "s": {"type": "multiple_choice", "options": ["the HR officer explains the process for requesting extended leave", "a broken bus stop", "a lost suitcase"], "answer": 0}}'::jsonb,
            '{"p": "Who gives the main explanation?", "p_de": "Who gives the main explanation?", "s": {"type": "multiple_choice", "options": ["HR Officer", "the cleaner", "the taxi driver"], "answer": 0}}'::jsonb,
            '{"p": "Which detail is named?", "p_de": "Which detail is named?", "s": {"type": "multiple_choice", "options": ["the employee must submit a formal written request at least four weeks in advance", "the door is painted blue", "the train leaves at midnight"], "answer": 0}}'::jsonb,
            '{"p": "Which counterpoint appears?", "p_de": "Which counterpoint appears?", "s": {"type": "multiple_choice", "options": ["the officer notes that urgent cases can be handled under a different policy", "nobody has a notebook", "the lesson is cancelled"], "answer": 0}}'::jsonb,
            '{"p": "What conclusion do the speakers reach?", "p_de": "What conclusion do the speakers reach?", "s": {"type": "multiple_choice", "options": ["the employee submits the request electronically and receives written confirmation", "they forget the topic completely", "they decide to leave the room"], "answer": 0}}'::jsonb
        ];
    BEGIN
...
DELETE FROM writing WHERE step_order=8990 AND path_uuid=v_path_id;
        ...
        VALUES (v_path_id, 8990, 'en', 'dialogue', 'professional', '[{"name": "Customer", "gender": "neutral", "avatarURL": "https://example.com/avatars/customer.png"}, {"name": "HR Officer", "gender": "neutral", "avatarURL": "https://example.com/avatars/clerk.png"}]'::jsonb)RETURNING uuid INTO v_dialogue_id;
