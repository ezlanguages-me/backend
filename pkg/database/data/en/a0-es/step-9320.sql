...
-- ============================================================
    ...
    DECLARE
        ...
        v_exercises JSONB[] := ARRAY[
            '{"p": "The dialogue is about a delegate expressing disagreement politely during a conference discussion.", "p_de": "The dialogue is about a delegate expressing disagreement politely during a conference discussion.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            '{"p": "The speakers are choosing food for the conference dinner.", "p_de": "The speakers are choosing food for the conference dinner.", "s": {"type": "true_false", "answer": false}}'::jsonb,
            '{"p": "Delegate mentions a conflicting study published in the same year.", "p_de": "Delegate mentions a conflicting study published in the same year.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            '{"p": "The speaker acknowledges the point and explains the limits of the original claim.", "p_de": "The speaker acknowledges the point and explains the limits of the original claim.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            '{"p": "Both speakers agree that the claim should be stated more carefully.", "p_de": "Both speakers agree that the claim should be stated more carefully.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            '{"p": "What is the main focus?", "p_de": "What is the main focus?", "s": {"type": "multiple_choice", "options": ["a delegate politely challenges a claim made in the keynote address", "an unrelated social chat", "a routine travel update"], "answer": 0}}'::jsonb,
            '{"p": "Who introduces the conflicting evidence?", "p_de": "Who introduces the conflicting evidence?", "s": {"type": "multiple_choice", "options": ["Delegate", "the cleaner", "the driver"], "answer": 0}}'::jsonb,
            '{"p": "Which detail is named?", "p_de": "Which detail is named?", "s": {"type": "multiple_choice", "options": ["a conflicting study from the same year", "the room is too cold", "the event starts late"], "answer": 0}}'::jsonb,
            '{"p": "What response does the speaker give?", "p_de": "What response does the speaker give?", "s": {"type": "multiple_choice", "options": ["the speaker acknowledges the point and qualifies the claim", "the sound system fails", "the audience falls asleep"], "answer": 0}}'::jsonb,
            '{"p": "What conclusion do the speakers reach?", "p_de": "What conclusion do the speakers reach?", "s": {"type": "multiple_choice", "options": ["the claim should be presented more carefully", "the topic is cancelled", "everyone leaves without speaking"], "answer": 0}}'::jsonb
        ];
    BEGIN
...
DELETE FROM writing WHERE step_order=9320 AND path_uuid=v_path_id;
        ...
        VALUES (v_path_id, 9320, 'en', 'dialogue', 'academic', '[{"name": "Delegate", "gender": "neutral", "avatarURL": "https://example.com/avatars/delegate.png"}, {"name": "Speaker", "gender": "neutral", "avatarURL": "https://example.com/avatars/speaker.png"}]'::jsonb)RETURNING uuid INTO v_dialogue_id;
        INSERT INTO dialogue_translation (dialogue_uuid, language, title) VALUES (v_dialogue_id, 'es', 'Diálogo con desacuerdo');
        INSERT INTO dialogue_translation (dialogue_uuid, language, title) VALUES (v_dialogue_id, 'de', 'Dialog mit Widerspruch');
