...
BEGIN
  ...
  VALUES (v_path_uuid, 9160, 'en')
  RETURNING uuid INTO v_writing_uuid;

  INSERT INTO writing_translation (writing_uuid, language, title, description, prompt)
  VALUES (v_writing_uuid, 'es', 'make notes on unfamiliar matters, but IS LIKELY to make notes which contain inaccuracies (Reuniones y Presentaciones)', 'Práctica guiada de writing: make notes on unfamiliar matters, but IS LIKELY to make notes which contain inaccuracies (Reuniones y Presentaciones).', '{"instruction":"make notes on unfamiliar matters, but IS LIKELY to make notes which contain inaccuracies (Reuniones y Presentaciones)","context":"Contenido pendiente.","language":"es"}'::jsonb);
END;
$seed$;
