...
DELETE FROM writing WHERE step_order=8960 AND path_uuid=v_path_id;
        ...
        VALUES (v_path_id, 8960, 'en', 'speaking', 'professional')RETURNING uuid INTO v_speaking_id;
        INSERT INTO speaking_translation (speaking_uuid, language, title, prompt)
        VALUES (v_speaking_id, 'es', 'Outside own immediate area of', '{"scenario": "Debes trabajar en inglés sobre este tema: the manager contacts the legal team to clarify the liability clause in a contract. El contexto es a corporate legal office.", "tasks": ["Presenta el problema principal.", "Indica el producto o documento implicado.", "Explica un detalle concreto del caso.", "Menciona la otra preocupación o límite.", "Pide una solución clara.", "Negocia una condición o confirma un requisito.", "Resume el acuerdo o el siguiente paso.", "Cierra de forma educada y profesional."]}'::jsonb);
        INSERT INTO speaking_translation (speaking_uuid, language, title, prompt)
        VALUES (v_speaking_id, 'de', 'Outside own immediate area of', '{"scenario": "Du sollst auf Englisch zu diesem Thema arbeiten: the manager contacts the legal team to clarify the liability clause in a contract. Der Kontext ist a corporate legal office.", "tasks": ["Stelle das Hauptproblem vor.", "Nenne das betroffene Produkt oder Dokument.", "Erkläre ein konkretes Detail des Falls.", "Erwähne die andere Sorge oder Grenze.", "Bitte um eine klare Lösung.", "Verhandle eine Bedingung oder bestätige eine Anforderung.", "Fasse die Vereinbarung oder den nächsten Schritt zusammen.", "Schließe höflich und professionell ab."]}'::jsonb);
    END; $seed$;
