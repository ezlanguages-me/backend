-- ============================================================
-- Seed: A0 English Path – STEP 8270 – Speaking – present and support own arguments (Conferencias, Seminarios y Clases)
...
DELETE FROM exercise WHERE target_uuid IN (SELECT uuid FROM reading WHERE step_order=8270 AND path_uuid=v_path_id);
DELETE FROM exercise WHERE target_uuid IN (SELECT uuid FROM listening WHERE step_order=8270 AND path_uuid=v_path_id);
DELETE FROM exercise WHERE target_uuid IN (SELECT uuid FROM dialogue WHERE step_order=8270 AND path_uuid=v_path_id);
DELETE FROM reading WHERE step_order=8270 AND path_uuid=v_path_id;
DELETE FROM listening WHERE step_order=8270 AND path_uuid=v_path_id;
