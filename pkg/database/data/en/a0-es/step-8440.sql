-- ============================================================
-- Seed: A0 English Path – STEP 8440 – Reading – understand a complex question (Conferencias, Seminarios y Clases)
...
DELETE FROM exercise WHERE target_uuid IN (SELECT uuid FROM reading WHERE step_order=8440 AND path_uuid=v_path_id);
DELETE FROM exercise WHERE target_uuid IN (SELECT uuid FROM listening WHERE step_order=8440 AND path_uuid=v_path_id);
DELETE FROM exercise WHERE target_uuid IN (SELECT uuid FROM dialogue WHERE step_order=8440 AND path_uuid=v_path_id);
DELETE FROM reading WHERE step_order=8440 AND path_uuid=v_path_id;
DELETE FROM listening WHERE step_order=8440 AND path_uuid=v_path_id;
DELETE FROM dialogue WHERE step_order=8440 AND path_uuid=v_path_id;
DELETE FROM speaking WHERE step_order=8440 AND path_uuid=v_path_id;
DELETE FROM writing WHERE step_order=8440 AND path_uuid=v_path_id;
        INSERT INTO reading (path_uuid,step_order,source_language,type,category,content)
        VALUES (v_path_id,8440,'en','reading','academic','Understand A Complex Question
...
In the end, the writer argues that the speaker promises a short pilot with extra data. The closing line states, "A good follow-up question opens the next layer of detail."')
        ...
        INSERT INTO reading_translation (reading_uuid,language,title,description) VALUES (v_reading_id,'es','understand a complex question','Lee el texto y responde.');
        INSERT INTO reading_translation (reading_uuid,language,title,description) VALUES (v_reading_id,'de','understand a complex question','Lies den Text und beantworte die Fragen.');
