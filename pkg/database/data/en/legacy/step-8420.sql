-- ============================================================
-- Seed: A0 English Path – STEP 8420 – Dialogue – probe for more detail in Q&A (Conferencias, Seminarios y Clases)
...
-- ============================================================
    ...
    DECLARE
        ...
        v_exercises JSONB[] := ARRAY[
            '{"p": "The dialogue is about a presentation on recycled packaging for campus cafés.", "p_de": "The dialogue is about a presentation on recycled packaging for campus cafés.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            ...
            '{"p": "Diego explains that questions ask about cost, supplier reliability and student feedback.", "p_de": "Diego explains that questions ask about cost, supplier reliability and student feedback.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            '{"p": "A counterpoint is that one audience member worries about moisture resistance.", "p_de": "A counterpoint is that one audience member worries about moisture resistance.", "s": {"type": "true_false", "answer": true}}'::jsonb,
            ...
            '{"p": "What is the main focus?", "p_de": "What is the main focus?", "s": {"type": "multiple_choice", "options": ["a presentation on recycled packaging for campus cafés", "a broken bus stop", "a lost suitcase"], "answer": 0}}'::jsonb,
            ...
            '{"p": "Which detail is named?", "p_de": "Which detail is named?", "s": {"type": "multiple_choice", "options": ["questions ask about cost, supplier reliability and student feedback", "the door is painted blue", "the train leaves at midnight"], "answer": 0}}'::jsonb,
            '{"p": "Which counterpoint appears?", "p_de": "Which counterpoint appears?", "s": {"type": "multiple_choice", "options": ["one audience member worries about moisture resistance", "nobody has a notebook", "the lesson is cancelled"], "answer": 0}}'::jsonb,
            '{"p": "What conclusion do the speakers reach?", "p_de": "What conclusion do the speakers reach?", "s": {"type": "multiple_choice", "options": ["the speaker promises a short pilot with extra data", "they forget the topic completely", "they decide to leave the room"], "answer": 0}}'::jsonb
        ];
    BEGIN
        SELECT uuid INTO v_path_id FROM path WHERE source_language = 'en' LIMIT 1;
DELETE FROM exercise WHERE target_uuid IN (SELECT uuid FROM reading WHERE step_order=8420 AND path_uuid=v_path_id);
DELETE FROM exercise WHERE target_uuid IN (SELECT uuid FROM listening WHERE step_order=8420 AND path_uuid=v_path_id);
DELETE FROM exercise WHERE target_uuid IN (SELECT uuid FROM dialogue WHERE step_order=8420 AND path_uuid=v_path_id);
DELETE FROM reading WHERE step_order=8420 AND path_uuid=v_path_id;
DELETE FROM listening WHERE step_order=8420 AND path_uuid=v_path_id;
DELETE FROM dialogue WHERE step_order=8420 AND path_uuid=v_path_id;
DELETE FROM speaking WHERE step_order=8420 AND path_uuid=v_path_id;
DELETE FROM writing WHERE step_order=8420 AND path_uuid=v_path_id;
        INSERT INTO dialogue (path_uuid, step_order, source_language, type, category, characters)
        VALUES (v_path_id, 8420, 'en', 'dialogue', 'academic', '[{"name": "Sara", "gender": "female", "avatarURL": "https://example.com/avatars/sara.png"}, {"name": "Diego", "gender": "male", "avatarURL": "https://example.com/avatars/diego.png"}]'::jsonb)RETURNING uuid INTO v_dialogue_id;
        INSERT INTO dialogue_translation (dialogue_uuid, language, title) VALUES (v_dialogue_id, 'es', 'Explorar respuestas en detalle');
        INSERT INTO dialogue_translation (dialogue_uuid, language, title) VALUES (v_dialogue_id, 'de', 'Antworten genauer erfragen');
