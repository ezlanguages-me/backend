...
DELETE FROM writing WHERE step_order=8540 AND path_uuid=v_path_id;
        ...
        VALUES (v_path_id, 8540, 'en', 'writing', 'academic')RETURNING uuid INTO v_writing_id;
        INSERT INTO writing_translation (writing_uuid, language, title, prompt)
        VALUES (v_writing_id, 'es', 'Present and support arguments', '{"scenario": "Debes trabajar en inglés sobre este tema: how to plan an essay on weekly reflection journals in first-year courses. El contexto es an academic writing workshop.", "tasks": ["Presenta el tema central.", "Explica el contexto académico o profesional.", "Resume la idea principal.", "Añade una evidencia o detalle importante.", "Menciona una objeción o alternativa.", "Responde a esa objeción.", "Formula una conclusión clara.", "Cierra con una recomendación breve."]}'::jsonb);
        INSERT INTO writing_translation (writing_uuid, language, title, prompt)
        VALUES (v_writing_id, 'de', 'Present and support arguments', '{"scenario": "Du sollst auf Englisch zu diesem Thema arbeiten: how to plan an essay on weekly reflection journals in first-year courses. Der Kontext ist an academic writing workshop.", "tasks": ["Stelle das zentrale Thema vor.", "Erkläre den akademischen oder beruflichen Kontext.", "Fasse die Hauptidee zusammen.", "Nenne einen wichtigen Beleg oder ein Detail.", "Erwähne einen Einwand oder eine Alternative.", "Reagiere auf diesen Einwand.", "Formuliere eine klare Schlussfolgerung.", "Beende mit einer kurzen Empfehlung."]}'::jsonb);
    END; $seed$;
