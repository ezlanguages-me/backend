    ...
    DECLARE
        ...
        v_exercises JSONB[] := ARRAY[
            '{"p": "The dialogue is about a delegate notices the subtle irony in a speaker''s closing remarks at a conference.", "p_de": "The dialogue is about a delegate notices the subtle irony in a speaker''s closing remarks at a conference.", "s": {"type": "true_false", "answer": true}}'::jsonb,
        ...
        '{"p": "Delegate explains that the speaker says ''that was quite interesting'' but her tone implies strong disagreement.", "p_de": "Delegate explains that the speaker says ''that was quite interesting'' but her tone implies strong disagreement.", "s": {"type": "true_false", "answer": true}}'::jsonb,
        '{"p": "A counterpoint is that another attendee interprets the remark as a sincere compliment.", "p_de": "A counterpoint is that another attendee interprets the remark as a sincere compliment.", "s": {"type": "true_false", "answer": true}}'::jsonb,
        ...
        '{"p": "What is the main focus?", "p_de": "What is the main focus?", "s": {"type": "multiple_choice", "options": ["a delegate notices the subtle irony in a speaker''s closing remarks at a conference", "a broken bus stop", "a lost suitcase"], "answer": 0}}'::jsonb,
        '{"p": "Who gives the main explanation?", "p_de": "Who gives the main explanation?", "s": {"type": "multiple_choice", "options": ["Delegate", "the cleaner", "the taxi driver"], "answer": 0}}'::jsonb,
        '{"p": "Which detail is named?", "p_de": "Which detail is named?", "s": {"type": "multiple_choice", "options": ["the speaker says ''that was quite interesting'' but her tone implies strong disagreement", "the door is painted blue", "the train leaves at midnight"], "answer": 0}}'::jsonb,
        '{"p": "Which counterpoint appears?", "p_de": "Which counterpoint appears?", "s": {"type": "multiple_choice", "options": ["another attendee interprets the remark as a sincere compliment", "nobody has a notebook", "the lesson is cancelled"], "answer": 0}}'::jsonb,
        '{"p": "What conclusion do the speakers reach?", "p_de": "What conclusion do the speakers reach?", "s": {"type": "multiple_choice", "options": ["the delegate explains that British understatement often means the opposite of what is said", "they forget the topic completely", "they decide to leave the room"], "answer": 0}}'::jsonb
        ];
    BEGIN
...
DELETE FROM writing WHERE step_order=9660 AND path_uuid=v_path_id;
        ...
        VALUES (v_path_id, 9660, 'en', 'dialogue', 'social', '[{"name": "Speaker A", "gender": "neutral", "avatarURL": "https://example.com/avatars/speaker-a.png"}, {"name": "Delegate", "gender": "neutral", "avatarURL": "https://example.com/avatars/delegate.png"}]'::jsonb)RETURNING uuid INTO v_dialogue_id;

        INSERT INTO dialogue_translation (dialogue_uuid, language, title) VALUES (v_dialogue_id, 'es', 'Up nuances of meaning/opinion');
        INSERT INTO dialogue_translation (dialogue_uuid, language, title) VALUES (v_dialogue_id, 'de', 'Up nuances of meaning/opinion');
