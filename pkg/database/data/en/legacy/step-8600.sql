...
DELETE FROM writing WHERE step_order=8600 AND path_uuid=v_path_id;
        ...
        VALUES (v_path_id, 8600, 'en', 'writing', 'academic')RETURNING uuid INTO v_writing_id;
        INSERT INTO writing_translation (writing_uuid, language, title, prompt)
        VALUES (v_writing_id, 'es', 'Conventional report structure', '{"scenario": "Debes trabajar en inglés sobre este tema: a short academic report on peer discussion and quiz results. El contexto es a study skills module.", "tasks": ["Presenta el tema central.", "Explica el contexto académico o profesional.", "Resume la idea principal.", "Añade una evidencia o detalle importante.", "Menciona una objeción o alternativa.", "Responde a esa objeción.", "Formula una conclusión clara.", "Cierra con una recomendación breve."]}'::jsonb);
        INSERT INTO writing_translation (writing_uuid, language, title, prompt)
        VALUES (v_writing_id, 'de', 'Conventional report structure', '{"scenario": "Du sollst auf Englisch zu diesem Thema arbeiten: a short academic report on peer discussion and quiz results. Der Kontext ist a study skills module.", "tasks": ["Stelle das zentrale Thema vor.", "Erkläre den akademischen oder beruflichen Kontext.", "Fasse die Hauptidee zusammen.", "Nenne einen wichtigen Beleg oder ein Detail.", "Erwähne einen Einwand oder eine Alternative.", "Reagiere auf diesen Einwand.", "Formuliere eine klare Schlussfolgerung.", "Beende mit einer kurzen Empfehlung."]}'::jsonb);
    END; $seed$;
