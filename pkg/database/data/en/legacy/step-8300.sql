...
DELETE FROM writing WHERE step_order=8300 AND path_uuid=v_path_id;
        ...
        VALUES (v_path_id, 8300, 'en', 'speaking', 'academic')RETURNING uuid INTO v_speaking_id;
        INSERT INTO speaking_translation (speaking_uuid, language, title, prompt)
        VALUES (v_speaking_id, 'es', 'Debate evaluativo', '{"scenario": "Debes trabajar en inglés sobre este tema: an attendance policy that rewards preparation rather than simple presence. El contexto es a policy seminar.", "tasks": ["Presenta el tema central.", "Explica el contexto académico o profesional.", "Resume la idea principal.", "Añade una evidencia o detalle importante.", "Menciona una objeción o alternativa.", "Responde a esa objeción.", "Formula una conclusión clara.", "Cierra con una recomendación breve."]}'::jsonb);
        INSERT INTO speaking_translation (speaking_uuid, language, title, prompt)
        VALUES (v_speaking_id, 'de', 'Bewertend diskutieren', '{"scenario": "Du sollst auf Englisch zu diesem Thema arbeiten: an attendance policy that rewards preparation rather than simple presence. Der Kontext ist a policy seminar.", "tasks": ["Stelle das zentrale Thema vor.", "Erkläre den akademischen oder beruflichen Kontext.", "Fasse die Hauptidee zusammen.", "Nenne einen wichtigen Beleg oder ein Detail.", "Erwähne einen Einwand oder eine Alternative.", "Reagiere auf diesen Einwand.", "Formuliere eine klare Schlussfolgerung.", "Beende mit einer kurzen Empfehlung."]}'::jsonb);
    END; $seed$;
