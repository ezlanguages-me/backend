...
DELETE FROM writing WHERE step_order=8380 AND path_uuid=v_path_id;
        ...
        VALUES (v_path_id, 8380, 'en', 'speaking', 'academic')RETURNING uuid INTO v_speaking_id;
        INSERT INTO speaking_translation (speaking_uuid, language, title, prompt)
        VALUES (v_speaking_id, 'es', 'take part in a debate', '{"scenario": "Debes trabajar en inglés sobre este tema: whether phones should be banned in seminars. El contexto es the student union debate night.", "tasks": ["Presenta el tema central.", "Explica el contexto académico o profesional.", "Resume la idea principal.", "Añade una evidencia o detalle importante.", "Menciona una objeción o alternativa.", "Responde a esa objeción.", "Formula una conclusión clara.", "Cierra con una recomendación breve."]}'::jsonb);
        INSERT INTO speaking_translation (speaking_uuid, language, title, prompt)
        VALUES (v_speaking_id, 'de', 'take part in a debate', '{"scenario": "Du sollst auf Englisch zu diesem Thema arbeiten: whether phones should be banned in seminars. Der Kontext ist the student union debate night.", "tasks": ["Stelle das zentrale Thema vor.", "Erkläre den akademischen oder beruflichen Kontext.", "Fasse die Hauptidee zusammen.", "Nenne einen wichtigen Beleg oder ein Detail.", "Erwähne einen Einwand oder eine Alternative.", "Reagiere auf diesen Einwand.", "Formuliere eine klare Schlussfolgerung.", "Beende mit einer kurzen Empfehlung."]}'::jsonb);
    END; $seed$;
