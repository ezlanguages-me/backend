...
DELETE FROM writing WHERE step_order=8180 AND path_uuid=v_path_id;
        ...
        VALUES (v_path_id, 8180, 'en', 'speaking', 'academic')RETURNING uuid INTO v_speaking_id;
        INSERT INTO speaking_translation (speaking_uuid, language, title, prompt)
        VALUES (v_speaking_id, 'es', 'Own opinion and justify it', '{"scenario": "Debes trabajar en inglés sobre este tema: a seminar article about urban gardens and local health. El contexto es a community studies seminar.", "tasks": ["Presenta el tema central.", "Explica el contexto académico o profesional.", "Resume la idea principal.", "Añade una evidencia o detalle importante.", "Menciona una objeción o alternativa.", "Responde a esa objeción.", "Formula una conclusión clara.", "Cierra con una recomendación breve."]}'::jsonb);
        INSERT INTO speaking_translation (speaking_uuid, language, title, prompt)
        VALUES (v_speaking_id, 'de', 'Own opinion and justify it', '{"scenario": "Du sollst auf Englisch zu diesem Thema arbeiten: a seminar article about urban gardens and local health. Der Kontext ist a community studies seminar.", "tasks": ["Stelle das zentrale Thema vor.", "Erkläre den akademischen oder beruflichen Kontext.", "Fasse die Hauptidee zusammen.", "Nenne einen wichtigen Beleg oder ein Detail.", "Erwähne einen Einwand oder eine Alternative.", "Reagiere auf diesen Einwand.", "Formuliere eine klare Schlussfolgerung.", "Beende mit einer kurzen Empfehlung."]}'::jsonb);
    END; $seed$;
